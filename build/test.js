...
var STATIC_BASE = 1024,
    ...
    STACKTOP = STACK_BASE,
    STACK_MAX = 360224,
    DYNAMIC_BASE = 5603104,
    DYNAMICTOP_PTR = 359264;
...
var ASM_CONSTS = {
  266898: function() {postMessage({cmd : 'processQueuedMainThreadWork'})},  
 266949: function($0) {if (!ENVIRONMENT_IS_PTHREAD) { if (!PThread.pthreads[$0] || !PThread.pthreads[$0].worker) { return 0; } PThread.pthreads[$0].worker.postMessage({cmd : 'processThreadQueue'}); } else { postMessage({targetThread : $0, cmd : 'processThreadQueue'}); } return 1;},  
 267317: function() {return !!(Module['canvas'])},  
 267353: function() {noExitRuntime = true}
};
...
// {{PRE_LIBRARY}}
  ...
  var __main_thread_futex_wait_address=360208;function _emscripten_futex_wake(addr, count) {
  ...
  function __cleanup_thread(pthread_ptr) {
      ...
      if (pthread) {
        ...
        PThread.mainThreadBlock = 359424;
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(11, 1, func, arg);
  ...
  function _emscripten_syscall(which, varargs) {
    switch (which) {
      ...
      case 200: return ___syscall200(which, varargs);
      case 201: return ___syscall201(which, varargs);
      case 202: return ___syscall202(which, varargs);
...
var proxiedFunctionTable = [null,___syscall195,___syscall197,___syscall202,___syscall20,___syscall220,___syscall221,___syscall3,___syscall5,___syscall54,___syscall91,_atexit,_emscripten_set_canvas_element_size_main_thread,_fd_close,_fd_fdstat_get,_fd_read,_fd_seek,_fd_write];
...
var real__crypto_keypair = asm["crypto_keypair"];
asm["crypto_keypair"] = function() {
  assert(runtimeInitialized, 'you need to wait for the runtime to be ready (e.g. wait for main() to be called)');
  assert(!runtimeExited, 'the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)');
  return real__crypto_keypair.apply(null, arguments);
};
...
var _crypto_keypair = Module["_crypto_keypair"] = function() {
  assert(runtimeInitialized, 'you need to wait for the runtime to be ready (e.g. wait for main() to be called)');
  assert(!runtimeExited, 'the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)');
  return Module["asm"]["crypto_keypair"].apply(null, arguments)
};
